...
# In-memory storage for status checks (replaces database)
status_checks_storage: List[dict] = []
...
async def create_status_check(input: StatusCheckCreate):
    ...
    status_checks_storage.append(doc)
    return status_obj

@api_router.get("/status", response_model=List[StatusCheck])
async def get_status_checks():
    # Return in-memory status checks
    status_checks = []
    for check in status_checks_storage:
        ...
        status_checks.append(StatusCheck(**check_copy))
